...
这一章节讨论了我们之前的玩具问题和开发产品的比较。下面是针对这个方法的3点意见。
...
在先前的研究中，转移对象是被定义来在控制器和视图间传递输入输出值，以减小对框架的依赖，重用逻辑控制器。

在这个实验中，使用转移对象能够减小对视图方法的依赖，以从实际观点上来看减小对框架的依赖。拥有像调用逻辑控制器和使用if语句和循环语句和创建对象的高功能的视图，增加了依赖，因为难以区别开视图和视图控制器的责任。

但是有一种情况，需要在页面和实体域和一个具有暂时功能的对象显示相同的事情，是一个缺点，因为多余的类定义增加了管理源代码的复杂度。在这种情况下，重用的影响不会通过使用传输展现，因为在几个用例里存在的少数类。

##### 逻辑控制器和视图控制器

这个方法的核心想法是使用外观模式创建元数据模型。在先前的研究当中，逻辑控制器不依赖于框架元素和从活动图创建的处理流程。在处理流程中，有多个输入和单个输出和异常。

在这个实验里，核心要点是在章节4.3后带来的影响。但是改变的点是处理流程可以返回多个输出，因为实际上的多个输出。通过元模型构成逻辑控制器和服务，要管理很多源代码文件是很困难的。所以我们构造一个包含类来包含逻辑控制器和视图控制器。

在处理流程中，在活动图每一个系统分区的动作分配给了实体方法。在这个实验中，活动有两种模式。在处理流程中，在活动图每一个系统分区的动作作为实体方法进行分配。但是在这个实验中，有三种类型的动作。（1）一个集成到其他动作中的动作。（2）一个对于处理流程拥有源码的动作，像是在java里的instanceof语句。（3）一个清楚映射到实体方法的动作。分配实体方法的缺点在于在多个动作步骤里的冗长描述无法聚集起来。对于开发者，很难为逻辑提取公共点和工具，因为一个动作只有输入和输出是清楚的。

### 贯穿改法的问题

##### 首先，讨论持久化模型

在活动流程图的系统分区里的全部对象节点被当成是单个用户使用的。但是需要说清楚哪一些数据被多个用户储存。所以所有的对象节点有4种功能。在系统分区的动作和源代码的持久化机制，像是JPA和通过SessionManager管理的Session和实体方法是一致的。所以处理流程不是不依赖框架的指定元素。在未来，将模型从持久化机制中分离，对于软件的可重用性和灵活性是十分必要的。

##### 接着，讨论视图和授权

视图是视图表面的模板，还有从视图控制器得来的数据产生页面。在这个实验中，视图是是跟复杂的，因为包含了两种授权（老师和学生），在用一个模板文件中以响应灵活的变更。视图对于授权有许多考虑的点。所以我们能够对这一点进行组织。

##### 最后，讨论知名的组件，像是分页等

当一个应用打算实现web框架，我们想要从知名的组件中选择，从而比较容易创建web应用。所以我们为了web能够组织知名组件集合。